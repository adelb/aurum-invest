...
/**
 * v9: the VIX is a first-class Wealth figure — its level, its 5-session
 * drift, and its regime label must survive the cache round-trip, and the
 * regime bands must stay glued to the score's own vixPoints thresholds.
 */
class MarketPulseVixTest {
    ...
    @Test
    fun driftWithoutLevelIsStillCarried() {
        // Level from the quote failed but candles served: the engine only
        // emits a drift when the level exists, but the serializer must not
        // conflate the two nulls.
