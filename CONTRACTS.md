...
`PortfolioGrader.grade(verdicts, book, flow, pulse): PortfolioGrade` (`analytics/PortfolioGrade.kt`) — the verified book scored on FIXED scales against seven named elite-investor rules: concentration control 20 (Buffett/position+sector caps), loss discipline 15 (O'Neil 8% rule), winners riding 15 (Livermore), trend alignment 15 (Weinstein 50-day), relative strength 15 (O'Neil vs SPY), money-flow alignment 10 (institutional), regime fit 10 (Livermore/tape). Unmeasurable disciplines are excluded from BOTH sides of the score and labeled "not measured" (score shown as `score`/`maxScore`); the weakest measured discipline produces the single concrete `suggestion`. Carried on `PortfolioReview.grade`, rendered by `PortfolioGradeCard` under the review summary, with the explicit caveat that this compares against published rules, not live elite holdings. `HoldingVerdict` carries the structured inputs: `above50: Boolean?`, `rel20Pct: Double?`, `flowVerdictName: String`.
